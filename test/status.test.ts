...
function event(
  overrides: Partial<CareerEvent> & Pick<CareerEvent, "type">,
): CareerEvent {
  return {
...
describe("calculateCurrentStatus", () => {
  test("履歴がなければ未着手になる", () => {
    expect(calculateCurrentStatus([])).toEqual({
      code: "not_started",
      label: "未着手",
    });
  });
  ...
  test("最新の事実から待ち状態を導出する", () => {
    ...
    expect(calculateCurrentStatus(events)).toEqual({
      code: "selection_waiting",
      label: "2次選考待ち",
    });
  });
