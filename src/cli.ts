...
export async function runCli(
  rawArgs: string[],
  output: CliOutput = terminalOutput,
): Promise<number> {
  let repository: CareerRepository | undefined;
  try {
    const global = extractGlobalArguments(rawArgs);
    if (
      global.args.length === 0 ||
      global.args.includes("--help") ||
      global.args.includes("-h")
    ) {
      output.write(HELP);
      return 0;
    }
    ...
    output.write(
      global.json ? JSON.stringify(result.value, null, 2) : result.text,
    );
    return 0;
  } catch (error) {
    if (
...
async function dispatch(
  ...
  if (!resource || !action)
    throw new ValidationError(
      "コマンドが不足しています。--helpで使い方を確認してください。",
    );
...
function companyCommand(
  ...
  switch (action) {
    case "add": {
      ...
      return {
        value: company,
        text: `企業を登録しました: ${company.name} (${shortId(company.id)})`,
      };
    }
    case "list": {
      ...
      return {
        ...
        text: companies.length
          ? companies
              .map(
                (company) =>
                  `${shortId(company.id)}\t${company.name}\t${company.status.label}`,
              )
              .join("\n")
          : "企業はまだ登録されていません。",
      };
    ...
    case "show": {
      assertAllowedOptions(parsed, []);
      assertPositionalCount(parsed, 1);
      const company = repository.findCompany(
      ...
      return {
        value: { company, status, events, notes, submissions },
        text: sections.join("\n"),
      };
    }
    case "update": {
      ...
      if (name === undefined && websiteRaw === undefined)
        throw new ValidationError("--nameまたは--websiteを指定してください。");
    ...
    case "delete": {
      ...
      if (relatedCount > 0 && !optionFlag(parsed, "force")) {
        throw new ConflictError(
          `企業には${relatedCount}件の関連データがあります。削除するには--forceを指定してください。`,
        );
      }
      repository.deleteCompany(company.id);
      return {
        value: { deleted: company },
        text: `企業を削除しました: ${company.name}`,
      };
    }
    default:
...
function eventCommand(
  ...
  if (action !== "add")
    throw new ValidationError(`未知のevent操作です: ${action}`);
  ...
  switch (type) {
    ...
    case "offer-received": {
      ...
      const input: Parameters<CareerRepository["addEvent"]>[0] = {
        type: "offer_received",
        companyId,
        occurredAt,
      };
      ...
      if (salary !== undefined)
        input.annualSalary = positiveNumber(salary, "--salary");
    ...
    case "resume-submitted":
      throw new ValidationError(
        "resume-submittedは`resume submit`コマンドで記録してください。",
      );
    default:
  ...
  return {
    value: event,
    text: `イベントを記録しました: ${formatEvent(event)}`,
  };
}
...
function noteCommand(
  ...
  switch (action) {
    case "add": {
      ...
      return {
        value: note,
        text: `ノートを追加しました: ${note.title} (${shortId(note.id)})`,
      };
    }
    case "list": {
      assertAllowedOptions(parsed, []);
    ...
    case "update": {
      ...
      if (title === undefined && body === undefined)
        throw new ValidationError("--titleまたは--bodyを指定してください。");
      const changes: { title?: string; body?: string } = {};
      if (title !== undefined) changes.title = title;
      if (body !== undefined) changes.body = body;
      const note = repository.updateNote(
        requiredPositional(parsed, 0, "ノートID"),
        changes,
      );
    ...
    case "delete": {
      assertAllowedOptions(parsed, []);
      assertPositionalCount(parsed, 1);
      const note = repository.findNote(
        requiredPositional(parsed, 0, "ノートID"),
      );
      repository.deleteNote(note.id);
      return {
        value: { deleted: note },
        text: `ノートを削除しました: ${note.title}`,
      };
    }
    default:
  ...
  switch (action) {
    case "add": {
      ...
      if (!(await file.exists()))
        throw new NotFoundError(`ファイル「${path}」が見つかりません。`);
      const defaultName = basename(path).replace(/\.pdf$/i, "");
      const resume = repository.addResume(
        optionValue(parsed, "name") ?? defaultName,
        new Uint8Array(await file.arrayBuffer()),
      );
      return {
        value: resume,
        text: `職務経歴書を登録しました: ${resume.name} (${shortId(resume.id)})`,
      };
    }
    case "list": {
      assertAllowedOptions(parsed, []);
    ...
    case "export": {
      assertAllowedOptions(parsed, ["force"]);
      assertPositionalCount(parsed, 2);
      const resume = repository.getResumeContent(
      ...
      if (existsSync(outputPath) && !optionFlag(parsed, "force")) {
        throw new ConflictError(
          `出力先「${outputPath}」は既に存在します。上書きするには--forceを指定してください。`,
        );
      }
      await Bun.write(outputPath, resume.content);
      const { content: _content, ...metadata } = resume;
      return {
        value: { resume: metadata, outputPath },
        text: `職務経歴書を書き出しました: ${outputPath}`,
      };
    }
    case "submit": {
      ...
      return {
        value: submission,
        text: `職務経歴書の提出を記録しました: ${shortId(submission.id)}`,
      };
    }
    case "delete": {
      assertAllowedOptions(parsed, ["force"]);
      assertPositionalCount(parsed, 1);
      const resume = repository.findResume(
        requiredPositional(parsed, 0, "職務経歴書IDまたは名前"),
      );
      repository.deleteResume(resume.id, optionFlag(parsed, "force"));
      return {
        value: { deleted: resume },
        text: `職務経歴書を削除しました: ${resume.name}`,
      };
    }
    default:
